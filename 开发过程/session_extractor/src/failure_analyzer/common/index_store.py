...
from src.util.agent_meta import load_agent_meta  # noqa: E402
...
class SessionIndex:
    ...
    def __init__(self, session_id: str, root: str | None = None):
        ...
        # 默认根目录：开发过程/projects-simplified
        # 本文件位于 src/tools/_common/index_store.py
        #   parents[0]=_common/  parents[1]=tools/  parents[2]=src/
        #   parents[3]=session_extractor/  parents[4]=开发过程/
        if root is None:
            root = str(Path(__file__).resolve().parents[4] / "projects-simplified")
        self.root = Path(root)
