"""
cli.py —— `python -m src.failure_analyzer <cmd> <args>` 形式 CLI（Bash 调用入口）
...
调用风格（<> 必填，[] 可选）：
    ...
    --help / -h（任意层级）：显示帮助文本
        python3.8 -m src.failure_analyzer --help           # 主 help
        python3.8 -m src.failure_analyzer find --help      # 某子命令详细 help
...
设计取舍：
    ...
    - 不静默吞错误（与 infra/scripts/subagent/*.py 一致）
...
def cmd_find(args: argparse.Namespace) -> int:
    ...
    if not args.pattern or args.list_patterns:
        ...
        root = args.root or str(Path(__file__).resolve().parents[3] / "projects-simplified")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "用法：python3.8 -m src.failure_analyzer <cmd> --help  查看子命令详细帮助。"
        ),
