...
- **主 agent 职责**：执行`PYTHONPATH=infra bash infra/scripts/with-python.sh infra/scripts/see-evolve.py <report.json> [skills_dir]`脚本 + 调度 evolver sub-agent
- **效果**：evolver agent 改 target_file + 输出 `evolution_report.json`
...
- 阶段 3 完成 = `evidence/evolution_reports/<session_id>.evolution_report.json` 存在
...
- **阶段 3 完**：
  - 报告 `evolution_report.json` 路径 + 已升级 / 失败统计
  - 流程结束
